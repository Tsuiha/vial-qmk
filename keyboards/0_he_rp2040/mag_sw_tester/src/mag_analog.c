...
void init_mux_select(void) {
	for (uint8_t i = 0; i < ARRAY_SIZE(mux_select_pins); i++) {
        palSetLineMode(mux_select_pins[i], PAL_MODE_OUTPUT_PUSHPULL);
    }
}
...
void set_mux_ch(uint8_t col) {
	for (uint8_t i = 0; i < ARRAY_SIZE(mux_select_pins); i++) {
		if (((mux_ch_order[col] >> i) & 1) != 0) {
			palSetLine(mux_select_pins[i]);
		} else {
			palClearLine(mux_select_pins[i]);
		}
	}
...
void enable_mux(uint8_t row) {
	palClearLine(mux_en_pins[row]);
}

void disable_mux(uint8_t row) {
	palSetLine(mux_en_pins[row]);
}

void init_mux_en(void) {
	for (uint8_t row = 0; row < MATRIX_ROWS; row++) {
		palSetLineMode(mux_en_pins[row], PAL_MODE_OUTPUT_PUSHPULL);
		disable_mux(row);
	}
}
...
/* Use polling wait instead of ChibiOS event wait for ADC completion */
#ifndef MAG_ADC_POLLING_WAIT
#define MAG_ADC_POLLING_WAIT
#endif
...
#ifndef MAG_ADC_POLLING_WAIT_LOOP_LIMIT
#define MAG_ADC_POLLING_WAIT_LOOP_LIMIT 10000
#endif
...
static volatile adc_conv_state_t adc_conv_state = ADC_CONV_IDLE;
...
static void adc_end_callback(ADCDriver *adcp) {
	(void)adcp;
#ifdef MAG_ADC_POLLING_WAIT
	adc_conv_state = ADC_CONV_DONE;
#else
    chSysLockFromISR();
...
void wait_adc_conv(void) {
	...
	while (adc_conv_state == ADC_CONV_BUSY && loop_count < MAG_ADC_POLLING_WAIT_LOOP_LIMIT) {
		__asm volatile("nop");
		loop_count++;
	}
	adc_conv_ok = (adc_conv_state == ADC_CONV_DONE);
#else
